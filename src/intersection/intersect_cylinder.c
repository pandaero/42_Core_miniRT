...
/*   Updated: 2023/02/01 18:49:25 by pbiederm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_intersect	*intersection_ray_cylinder(t_ray *ray, t_cylinder *cylinder)
{
	...
	else
	{
		...
		ft_printf("ray misses  infinite cylinder\n");
		return (intersection);
	}
	if (distance_from_base >= 0.0 && distance_from_base <= cylinder->height)
	{
		intersection->state = 1;
		ft_printf("intersects within cylinder caps\n");
		return (intersection);
	}
	intersection->state = 1;
	ft_printf("intersects within infinite cylinder\n");
	return(intersection);
}
